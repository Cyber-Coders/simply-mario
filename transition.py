...
def transition_menu(screen):
    ...
    Text = font_text1.render('Congratulations', 1, (0, 0, 0))
    Text_add = font_text2.render('you passed the level!', 1, (0, 0, 0))
    ...
    # Проверка и изменение цвета при наведение у кнопки Quit
    if x < mouse[0] < x + 100:
        if y < mouse[1] < y + 30:
    ...
    Quit = font.render('Quit', 1, backlight_Quit)
    screen.blit(Quit, (x, y))

    # Проверка и изменение цвета при наведение у кнопки Next
    ...
    Next = font.render('Next', 1, backlight_Next)
